...
## Workflow Designer sidebar behavior

- System and Custom workflow groups share one vertical scroll area.
- Horizontal scrolling is disabled; long workflow names use ellipsis.
- Sidebar workflow items show the workflow name only.

## Topbar action group

`Assets` and `+ New Workflow` share the same visual action group, so they should look like one grouped control instead of separate floating buttons.
