...
工程型 Workflow 在實作前先執行 Baseline；報告型 Security Vulnerability Scan 會略過此階段並直接開始盤點。需要 Baseline 時，原本就存在的失敗會留下 Evidence；最後只阻擋新增／惡化的失敗與未完成 Acceptance，不會自動要求 Agent 修掉所有無關歷史問題。
...
不要直接壓縮工作目錄。`scripts/build_release.py` 採白名單，並產生 ZIP 內的 `RELEASE_MANIFEST.json` 與外部 Sidecar Manifest。發版證據應保存 Sidecar，才能確認實際交付檔案與 Hash。

Runtime State 必須排除。備份與 Release ZIP 是兩種不同產物：SQLite／Settings／Profile 可為了復原另外備份，但不可隨產品 ZIP 對外散佈。

## 資料庫 Migration 維運

啟動時只會套用缺少的 SQLite Migration。既有 Schema 變更前會自動建立一份 `pre-migration` 備份；一般 Local 使用請勿設定 `AIWF_SQLITE_AUTO_BACKUP=0`。Migration 失敗會阻止啟動，且失敗版本不會寫入。`DATABASE_SCHEMA_TOO_NEW` 表示 DB 由較新版 Controller 建立，不可直接降版開啟。

升級後應檢查 `/ready`、`/api/health/deep`、`/api/maintenance/store/status`。需要 Rollback 時，先還原 pre-migration 備份，再啟動舊版程式。

## CommandRunner 維運

Project 命令的 cwd 必須位於宣告的 Project Path 內。Agent 產生命令只能使用 argv，禁止 Shell。輸出會統一 UTF-8、遮罩敏感資訊並限制大小；Timeout 會終止整個 Process Tree。`TIMEOUT`／`COMMAND_FAILED` 應由 Validation Artifact 診斷，不要在不同 cwd／環境下手動重跑後直接視為通過。
