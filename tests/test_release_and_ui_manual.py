...
class PlaywrightUiManualTests(unittest.TestCase):
    def test_playwright_ui_e2e_is_opt_in(self) -> None:
        ...
        port = os.environ.get("PLAYWRIGHT_TEST_PORT", "8765")
        ...
        try:
            ...
            self._run_playwright_script(base_url)
        finally:
            server.terminate()
            try:
    ...
    def _run_playwright_script(self, base_url: str) -> None:
        title = f"UI E2E Project {uuid.uuid4().hex[:8]}"
        script = textwrap.dedent(
            ...
            with sync_playwright() as p:
                ...
                if project_folder_name not in run_meta_text:
                    raise AssertionError(
                        '#runMeta should contain project folder name '
                        + repr(project_folder_name)
                        + ', actual='
                        + repr(run_meta_text)
                    )

                page.fill('#messageInput', 'Add a deterministic helper and tests.')
                expect(page.locator('#runWorkflow')).to_be_enabled(timeout=10000)
                page.click('#runWorkflow')
                expect(page.locator('#resultText')).not_to_have_text('Waiting', timeout=15000)
        ...
        proc = subprocess.run([sys.executable, "-c", script], cwd=str(ROOT), capture_output=True, text=True, timeout=180)
