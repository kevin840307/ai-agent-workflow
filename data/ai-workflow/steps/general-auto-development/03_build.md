...
Task manifest JSON generated by Python:
{{task_manifest_json}}

Compiled workflow instance for this run:
{{workflow_instance}}
...
- Implement the current small task from the compiled workflow instance. The runner will call you task-by-task; do not jump ahead unless the current task depends on shared setup. Assemble coherent project state across retries and later tasks.
