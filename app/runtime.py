...
- If you can complete the artifact with reasonable assumptions, do so and record assumptions or Unknowns in the artifact.
- If a missing decision would materially change the implementation, tests, scope, file layout, API shape, or user-facing behavior, ask the user before continuing.
...
- After the user replies, their reply will be provided as User-provided answers in the next prompt.
...
def select_skill_files(skill_path: str | None, step_key: str, requirement: str) -> list[Path]:
    ...
    exact = []
    for wanted in names_by_step.get(step_key, []):
...
def skill_runtime_guidance(skill_files: list[Path]) -> str:
    ...
    lines = [
        ...
        "If a missing decision blocks safe progress, you may output only ask_user_question JSON.",
...
async def run_qwen_step(run: dict[str, Any], step_key: str, prompt_name: str, artifact: str) -> None:
    ...
    if skill_context.strip():
        ...
        prompt = (
            ...
            "If you need user input, output only ask_user_question JSON."
        )
    if skill_context.strip():
